...
//>REMOVE-BRIDGE<
{
    //<REMOVE-BRIDGE>
    @IgnoreAutoComplete
    private static String FILE_FORMAT = FormatDictionaries.GEOMETRY_GLSL;

    @IgnoreAutoComplete
    public static ClassInspectorEntry inspectorController(){
        Class thisClass = GeometryGLSLFile.class;
        return new ClassInspectorEntry(new ClassInterface() {
            ...
            public Variable save(String fieldName, Object value, UserPointer userPointer) {
                GeometryGLSLFile file = (GeometryGLSLFile) value;
                if(file != null) {
            ...
            public Object restore(Variable variable, UserPointer userPointer) {
                if(variable.type == Variable.Type.File){
                    return new GeometryGLSLFile(variable.str_value);
                }
                return null;
            }
    ...
    public GeometryGLSLFile(String path) {
        super(path);
    }
    //>REMOVE-BRIDGE<

    private GeometryGLSLFile() {
        //<REMOVE-BRIDGE>
        super();
        //>REMOVE-BRIDGE<
    }
